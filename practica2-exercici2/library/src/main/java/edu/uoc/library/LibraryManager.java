...
public class LibraryManager {
    ...
    public void saveMonumentInBackground(
            ...
            final Bitmap image,
            final GetCallback<Monument> callback) {
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    final MonumentList monuments = gson.fromJson(FileUtils.getMonumentDatabaseContent(context), MonumentList.class);
                    // To use callback in the main thread
                    new Handler(context.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                ...
                try {
                    ...
                    if (!existMonumentId) {
                        new Handler(context.getMainLooper()).post(new Runnable() {
                            @Override
                            public void run() {
                                callback.onFailure(new RuntimeException("Incorrect monument id"));
                            }
                        });
                    }
                } catch (final Exception e) {
    ...
    private Monument getMonument(
        ...
        monument.setId(new Gson().fromJson(
                FileUtils.getMonumentDatabaseContent(context),
                MonumentList.class).getMonuments().size()+1);
        // Set name
